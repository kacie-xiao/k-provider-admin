  ...
  >
    ...
    >
      <div class="layout-navbars-breadcrumb-user-link">
        ...
        <!--        />-->
        <span v-if="userInfos.userName" class="text-lg" @click="gotoMineCenter">
          {{ userInfos.userName }}
        </span>
        ...
        <!--        </el-icon>-->
        <div class="ml-6">
          <n-button type="error" size="tiny" @click="logOut">退出登录</n-button>
        </div>
      </div>
      <!--      <template #dropdown>-->
      <!--        <el-dropdown-menu>-->
...
//去个人中心
function gotoMineCenter() {
  router.push({ path: "/mine/mine-center" });
}
...
function logOut() {
  $naiveDialog.error({
    ...
    showIcon: false,
    negativeText: t("message.user.logOutCancel"),
    positiveText: t("message.user.logOutConfirm"),
    onPositiveClick: () => {
      // 清除缓存/token等
      Session.clear();
