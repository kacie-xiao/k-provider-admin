...
const initTagsView = async () => {
  if (Session.get('tagsViewList') && getThemeConfig.value.isCacheTagsView) {
    ...
    await addTagsView(route.path, route as RouteItem);
  }
  // 初始化当前元素(li)的下标
...
const closeCurrentTagsView = (path: string, callback?: Function) => {
...
onBeforeMount(() => {
   ...
   */
  mittBus.on(
    'onCloseCurrentTagsViewClick',
    (options: { route: RouteLocationNormalizedLoaded; callback: Function; errorCallback: Function }) => {
      const url = setTagsViewHighlight(options.route);
      const data = state.tagsViewList.find((item) => {
        return item.url === url;
      });
      if (!data) return options.errorCallback();
      closeCurrentTagsView(getThemeConfig.value.isShareTagsView ? data.path : data.url, options.callback);
    },
  );
   ...
   * 刷新某一项
   */
  mittBus.on('onRefreshCurrentTagsView', (route: RouteLocationNormalizedLoaded) => {
    const item = state.tagsViewList.find((item) => item.name === route.name);
    if (item) {
      refreshCurrentTagsView(item.fullPath);
    }
  });
...
onUnmounted(() => {
   ...
   * 取消监听关闭某一项
   */
  mittBus.off('onCloseCurrentTagsViewClick', () => {});
  /**
   * 取消监听刷新某一项
   */
  mittBus.off('onRefreshCurrentTagsView', () => {});
  // 取消监听布局配置界面开启/关闭拖拽
